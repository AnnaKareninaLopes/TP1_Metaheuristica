...
class Mst:
    ...
    def __gen_edge_matrix(
        ...
        for node, (cordx, cordy) in enumerate(cordenates):
            ...
            for node2, (cordx2, cordy2) in enumerate(cordenates):
                if node != node2:
                    edge_list.append(
                        (((cordx-cordx2)**2 + (cordy-cordy2)**2)**0.5, node, node2)
                    )
    ...
    def __gen_mst(self, edge_matrix: dict[int, list[tuple[int, int]]], first_city: int):
        ...
        for _ in range(len(edge_matrix)-1):
            _, from_node, to_node = self.__get_first_unconnected(visited, adjacent_to_tree)
    ...
    def __init__(
        ...
        edge_matrix:dict[int, list[tuple[int, int]]] = self.__gen_edge_matrix(cordenates)
