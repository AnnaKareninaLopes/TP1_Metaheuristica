...
class ConstructiveHeuristic(ABC):
    ...
    def criar_matriz_distancias(self, coordenadas:list[list[int, int]]) -> list[list[int]]:
        ...
        for i in range(tamanho):
            for j in range(tamanho):
                if i != j:
                    matriz[i][j] = distancia_euclidiana_2d(coordenadas[i], coordenadas[j])

        return matriz

    def criar_dicionario_cidades_ordenadas(self, matriz_distancias: list[list[int]]) -> dict[int, list[int]]:
        dicionario = {}
